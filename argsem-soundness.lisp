...
(defun admissible-extension-p (graph arguments)
  (and (conflict-free-extension-p graph arguments)
       (every (curry #'acceptable-p graph arguments)
              arguments)))

(defun complete-extension-p (graph arguments)
  (with-member* (arguments)
    (and (admissible-extension-p graph arguments)
         (every (lambda (argument)
                  (implies (acceptable-p graph arguments argument)
                           (member* argument)))
                (graph:nodes graph)))))

(defun grounded-extension-p (graph arguments)
  (and (complete-extension-p graph arguments)
       (let ((sub-extensions
               (remove arguments (powerset arguments) :test #'set-equal)))
         (notany (curry #'complete-extension-p graph) sub-extensions))))

(defun stable-extension-p (graph arguments)
  (with-member* (arguments)
    (and (conflict-free-extension-p graph arguments)
         (every (lambda (argument)
                  (some (lambda* ((b a))
                          (and (eql a argument)
                               (member* b)))
                        (edges graph)))
                (set-difference (graph:nodes graph) arguments)))))

(defun preferred-extension-p (graph arguments)
  (and (complete-extension-p graph arguments)
       (every (lambda (set)
                (implies (and (not (set-equal arguments set))
                              (subsetp* arguments set))
